...
import {
  Layout, Database, Layers, FolderTree, Package, ChevronDown,
  Search, Sparkles,
} from 'lucide-react';
...
import type { TopNavType, ComponentType } from '@/types/editor';

const NAV_ITEMS: { key: TopNavType; icon: React.ComponentType<{ className?: string }>; label: string }[] = [
  { key: 'components', icon: Layout, label: '组件库' },
  { key: 'datasource', icon: Database, label: '数据源' },
  { key: 'ai-templates', icon: Sparkles, label: 'AI模板' },
...
export function TopNavBar() {
  ...
  const filteredTemplates = TEMPLATES.filter((t) => {
    return !templateSearch || t.name.includes(templateSearch) || t.industry.includes(templateSearch) || t.style.includes(templateSearch);
  });
  ...
  const handleNavClick = (key: TopNavType) => {
    setTopNav(key);
  };

  const renderDropdownContent = () => {
    if (!topNavDropdownOpen) return null;
                ...
                <div className="grid grid-cols-6 gap-3">
                  {filteredComponents.map((comp) => (
                    <div
                      ...
                      className="flex flex-col items-center gap-1.5 p-3 rounded-lg bg-[#0a0e1a] border border-[#1e293b] cursor-grab hover:border-cyan-500/40 hover:bg-cyan-500/5 transition-colors active:cursor-grabbing group"
                    >
            ...
            <div className="flex-1 overflow-y-auto p-4">
              <div className="grid grid-cols-5 gap-4">
                {filteredTemplates.map((tpl) => (
                  <div
                    key={tpl.id}
                    onClick={() => { applyTemplate(tpl); closeTopNavDropdown(); }}
                  ...
                  >
                    <div className="h-24 bg-gradient-to-br from-[#0c4a6e] to-[#1e293b] flex items-center justify-center relative">
                      ...
                      <span className="text-sm font-semibold text-cyan-400/80">{tpl.industry}</span>
                    </div>
              ...
              {components.length === 0 ? (
                ...
                <div className="grid grid-cols-6 gap-2">
                  {[...components].reverse().map((comp) => (
                    <div
                      key={comp.id}
                      className="flex items-center gap-2 px-3 py-2 rounded bg-[#0a0e1a] border border-[#1e293b] hover:border-cyan-500/40 cursor-pointer group text-xs"
                      onClick={() => useEditorStore.getState().selectComponent(comp.id)}
                    >
                      <DynamicIcon name="Square" className="w-3.5 h-3.5 text-slate-500 flex-shrink-0" />
            ...
            <div className="flex-1 flex items-center justify-center p-8">
              {dataSources.length === 0 ? (
                ...
                <div className="w-full max-w-[600px] space-y-2">
                  {dataSources.map((ds) => (
                    <div key={ds.id} className="flex items-center gap-3 p-3 rounded-lg bg-[#0a0e1a] border border-[#1e293b]">
                      <Database className="w-4 h-4 text-cyan-400" />
                      <div className="flex-1">
                        <div className="text-xs font-medium text-slate-200">{ds.name}</div>
                        <div className="text-[10px] text-slate-500">{ds.type} · {ds.status}</div>
                      </div>
                    </div>
                  ))}
                </div>
  ...
  return (
    <div className="bg-[#111827] border-b border-[#1e293b] select-none">
      {/* Navigation tabs row */}
        ...
        {NAV_ITEMS.map(({ key, icon: Icon, label }) => (
          <button
            ...
            className={cn(
              ...
              topNavActive === key && topNavDropdownOpen
                ? 'bg-cyan-500/20 text-cyan-400'
          ...
          >
            ...
            <ChevronDown
              className={cn(
                ...
                topNavActive === key && topNavDropdownOpen && 'rotate-180'
              )}
            />
          </button>
        ))}
        <button
          onClick={closeTopNavDropdown}
          className={cn(
            'ml-auto p-1 rounded text-slate-500 hover:text-slate-300 transition-colors',
            !topNavDropdownOpen && 'hidden'
          )}
        >
      ...
      {/* Dropdown panel */}
      {topNavDropdownOpen && (
        <div className="border-t border-[#1e293b] h-[220px] overflow-hidden">
